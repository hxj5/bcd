...
def get_snp_metrics_group(adata_lst, id_lst, X_names, metrics = None):
    assert len(adata_lst) == len(X_names)
    mv = __get_gw_metrics_group(
        X_lst = [adata.layers["DP"] for adata in adata_lst],
        id_lst = id_lst,
        X_names = X_names,
        metrics = metrics
    )
    mv_AD = __get_gw_metrics_group(
        X_lst = [adata.layers["AD"] for adata in adata_lst],
        id_lst = id_lst,
        X_names = X_names,
        metrics = metrics
    )    
    mv["DP"] = mv["sum"]
    mv["AD"] = mv_AD["sum"]
    mv["baf"] = get_baf(mv["DP"] - mv["AD"], mv["AD"])
